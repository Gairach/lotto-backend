...
exports.getUserPurchaseHistory = exports.claimReward = exports.getUserRewards = exports.closeDraw = exports.buyTicket = exports.getAvailableTickets = exports.createDraw = exports.getTickets = void 0;
const db_1 = require("../config/db");
// ดึงเลขหวยทั้งหมด (สำหรับดูข้อมูลตั๋วทั้งหมด)
const getTickets = async (req, res) => {
    try {
        const [rows] = await db_1.db.query("SELECT * FROM lotto_tickets");
        res.json(rows);
    }
    catch (err) {
...
function generateLottoNumbers(count = 100) {
...
const createDraw = async (req, res) => {
    try {
        ...
        const [result] = await db_1.db.query("INSERT INTO draws () VALUES ()");
        ...
        await db_1.db.query("INSERT INTO lotto_tickets (ticket_number, draw_id) VALUES ?", [values]);
...
const getAvailableTickets = async (req, res) => {
    try {
        ...
        const [rows] = await db_1.db.query("SELECT * FROM lotto_tickets WHERE draw_id = ? AND status = 'available'", [drawId]);
        res.json(rows);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
const buyTicket = async (req, res) => {
    try {
        ...
        const [[ticket]] = await db_1.db.query("SELECT price, status FROM lotto_tickets WHERE ticket_id = ?", [ticketId]);
        if (!ticket)
            return res.status(404).json({ error: "ไม่พบเลขนี้" });
        if (ticket.status !== "available") {
            return res.status(400).json({ error: "เลขนี้ถูกขายไปแล้ว" });
        }
        // 2. ดึง wallet ของผู้ใช้
        const [[user]] = await db_1.db.query("SELECT wallet FROM users WHERE user_id = ?", [user_id]);
        if (!user)
        ...
        await db_1.db.query("UPDATE lotto_tickets SET status = 'sold' WHERE ticket_id = ?", [ticketId]);
        // 4. หักเงินจาก wallet
        await db_1.db.query("UPDATE users SET wallet = wallet - ? WHERE user_id = ?", [ticket.price, user_id]);
        // 5. บันทึกการซื้อ
        await db_1.db.query("INSERT INTO purchases (user_id, ticket_id, purchase_date) VALUES (?, ?, NOW())", [user_id, ticketId]);
        res.json({
            message: "ซื้อสำเร็จ",
            ticketId,
            user_id,
...
const closeDraw = async (req, res) => {
    try {
        ...
        if (isNaN(drawId))
            return res.status(400).json({ error: "drawId ไม่ถูกต้อง" });
        // 1. อัพเดตสถานะงวดเป็น closed
        await db_1.db.query("UPDATE draws SET status = 'closed' WHERE draw_id = ?", [drawId]);
        // 2. ดึงเลขทั้งหมดของงวด
        const [tickets] = await db_1.db.query("SELECT ticket_number FROM lotto_tickets WHERE draw_id = ?", [drawId]);
        if (tickets.length < 5) {
            return res.status(400).json({ error: "จำนวนเลขไม่เพียงพอสำหรับสุ่มรางวัล" });
        }
        // 3. สุ่มเลขรางวัล 5 ตัว
        const shuffled = tickets.map((t) => t.ticket_number).sort(() => Math.random() - 0.5);
        ...
        for (const p of prizeData) {
            await db_1.db.query("INSERT INTO prizes (draw_id, prize_type, winning_number, reward_amount) VALUES (?, ?, ?, ?)", [drawId, p.type, p.number, p.reward]);
        }
        res.json({
...
const getUserRewards = async (req, res) => {
    try {
        ...
        // ดึงสถานะงวด
        const [draw] = await db_1.db.query("SELECT status FROM draws WHERE draw_id = ?", [drawId]);
        if (draw.length === 0)
            return res.status(404).json({ error: "ไม่พบนงวดนี้" });
        const drawStatus = draw[0].status; // active / closed
        // ดึง ticket ของผู้ใช้พร้อม purchase_id
        const [tickets] = await db_1.db.query(`SELECT t.ticket_number, t.ticket_id, p.purchase_id
       FROM lotto_tickets t
       JOIN purchases p ON t.ticket_id = p.ticket_id
       WHERE t.draw_id = ? AND p.user_id = ?`, [drawId, userId]);
        if (tickets.length === 0)
            return res.json({ drawStatus, rewards: [] });
        // ดึงรางวัลทั้งหมดของงวด
        const [prizes] = await db_1.db.query("SELECT * FROM prizes WHERE draw_id = ?", [drawId]);
        // ดึงข้อมูลตั๋วที่เคยขึ้นรางวัลแล้ว
        const [claims] = await db_1.db.query("SELECT purchase_id, prize_id FROM claims WHERE purchase_id IN (?)", [tickets.map((t) => t.purchase_id)]);
        // ตรวจสอบแต่ละ ticket ว่าถูกรางวัลหรือขึ้นรางวัลแล้ว
        const rewards = tickets.map((ticket) => {
            const prize = prizes.find((p) => p.winning_number === ticket.ticket_number);
            const claimed = prize
                ? claims.some((c) => c.purchase_id === ticket.purchase_id && c.prize_id === prize.prize_id)
                : false;
            return {
...
const claimReward = async (req, res) => {
    try {
        ...
        const [rows] = await db_1.db.query(`SELECT p.purchase_id, p.user_id, t.ticket_number, r.prize_id, r.reward_amount, r.prize_type
       FROM purchases p
       JOIN lotto_tickets t ON p.ticket_id = t.ticket_id
       JOIN prizes r ON r.draw_id = t.draw_id AND r.winning_number = t.ticket_number
       WHERE p.purchase_id = ? AND p.user_id = ?`, [purchaseId, userId]);
        ...
        if (rows.length === 0)
            return res.status(400).json({ error: "ไม่พบข้อมูลรางวัลที่ถูกรางวัล" });
        const prize = rows[0];
        // ตรวจสอบว่าขึ้นรางวัลไปแล้วหรือยัง
        const [checkClaim] = await db_1.db.query("SELECT * FROM claims WHERE purchase_id = ? AND prize_id = ?", [purchaseId, prize.prize_id]);
        if (checkClaim.length > 0)
            return res.status(400).json({ error: "ตั๋วนี้ขึ้นรางวัลไปแล้ว" });
        // Insert ข้อมูลการขึ้นรางวัล
        await db_1.db.query(`INSERT INTO claims (purchase_id, prize_id, claim_date, amount) 
       VALUES (?, ?, NOW(), ?)`, [purchaseId, prize.prize_id, prize.reward_amount]);
        // อัพเดต wallet ของผู้ใช้
        await db_1.db.query(`UPDATE users SET wallet = wallet + ? WHERE user_id = ?`, [prize.reward_amount, userId]);
        res.json({
...
const getUserPurchaseHistory = async (req, res) => {
    try {
       ...
       WHERE p.user_id = ?
       ORDER BY p.purchase_date DESC`, [userId]);
